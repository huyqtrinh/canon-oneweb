...
public class MyOrderPageActions {
    public static void SearchString(WebDriver driver, String sSearchKey) throws InterruptedException {
        MyActions.setTexts(MyOrdersPage.tbx_Search(driver), sSearchKey);
        Thread.sleep(1000);
        new MyOrdersPage().tbx_Search(driver).sendKeys(Keys.RETURN);
        Thread.sleep(1000);
    }
    public static void FilterOrder(WebDriver driver, String sOrderNum, String sInvoiceNum, String sCreatedBy,
        ...
        {
            if (MyActions.checkDisplayed(MyOrdersPage.btn_Filter(driver))) {
                MyActions.clickObject(MyOrdersPage.btn_Filter(driver));
                Thread.sleep(1000);
            }
            if (!sOrderNum.equals("")){MyActions.setTexts(MyOrdersPage.tb_Filter_OrderNumber(driver), sOrderNum); }
            if (!sInvoiceNum.equals("")){MyActions.setTexts(MyOrdersPage.tb_Filter_InvoiceNumber(driver), sInvoiceNum); }
            ...
            if (!sOrderDateFrom.equals("")) {MyActions.setTexts(MyOrdersPage.tb_Filter_OrderDateFrom(driver), sOrderDateFrom);}
            if (!sOrderDateTo.equals("")) { MyActions.setTexts(MyOrdersPage.tb_Filter_OrderDateTo(driver), sOrderDateTo); }
            if (!sOrderMin.equals("")) { MyActions.setTexts(MyOrdersPage.tb_Filter_OrderTotalMin(driver), sOrderMin); }
            if (!sOrderMax.equals("")) { MyActions.setTexts(MyOrdersPage.tb_Filter_OrderTotalMax(driver), sOrderMax); }
            Thread.sleep(1000);

            MyActions.clickObject(MyOrdersPage.btn_Apply(driver));
            Thread.sleep(1000);
        }
    ...
    public static void ClearFilterOrder(WebDriver driver) throws InterruptedException {
        if (MyActions.checkDisplayed(MyOrdersPage.href_ClearAll(driver))) {
            MyActions.clickObject(MyOrdersPage.href_ClearAll(driver));
            Thread.sleep(1000);
        }
    }
}
