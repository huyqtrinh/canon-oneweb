package Utilities;

public class Constants {
    public static boolean Emulator = false;
    public static String Browser = "Chrome";
    public static String MagentoQA_Username = "automation@gmail.com";
    public static String MagentoQA_Password = "Canon123";
    public static String MagentoQA_Url= "https://mcstaging-estore.canon.ca/en_ca/";

    public static String MagentoCCIAdmin_Username1 = "amiyapuram";
    public static String MagentoCCIAdmin_Password1 = "Canon123";
    public static String MagentoCCIAdmin_Username2 = "Gaurav123";
    public static String MagentoCCIAdmin_Password2 = "Canon123";
    public static String MagentoCCIAdmin_Url= "https://mcstaging-shop.usa.canon.com/admin";

    public static String MagentoOrderManagement_Username1 = "gdewan";
    public static String MagentoOrderManagement_Password1 = "C@non2021";
    public static String MagentoOrderManagement_Username2 = "Amiyapuram";
    public static String MagentoOrderManagement_Password2 = "Canon@123";
    public static String MagentoOrderManagement_Url= "https://canon-stg.mcom.magento.com/admin/login";

    public static String MyAccountPage_firstName = "Doe";
    public static String MyAccountPage_lastName = "John";

    public static String Wrong_Username = "mamabear@gmail.com";
    public static String Wrong_Password = "abcde123";
    public static String TestData_Path = "src/test/java/testData/";
}
