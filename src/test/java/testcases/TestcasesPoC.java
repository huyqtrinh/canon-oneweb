...
public class TestcasesPoC {
    ...
    public void TC_003_Verify_Create_and_move_Orders() {
        ...
        try {
            ...
            //Step 4: Click on the Product Category from mega menu (Click on Cameras > Mirrorless Cameras)
            Reporter.log("Step 4: Verify after click Product Category:", true);
            MyActions.clickObject(QAHomePage.btn_Cameras(driver));
            driver.manage().timeouts().pageLoadTimeout(60, TimeUnit.SECONDS);
            driver.navigate().to("https://mcstaging-estore.canon.ca/en_ca/cameras/mirrorless");
            driver.manage().timeouts().implicitlyWait(60, TimeUnit.SECONDS);
            strCurTitle = driver.getTitle();
            Assert.assertEquals(strCurTitle, "Shop Canon Mirrorless | Canon Canada, Inc.", "Not navigate to Compact Camera page.");
            Reporter.log("Navigated to Mirrorless Cameras", true);

            //Step 5: Click on Powershot SX540 HS from the Product List Page. This will navigate the user to Product Detail page (PDP)
            String sProductName = "EOS R Body";
